...
export default function DashboardLayout({children,}: Readonly<{children: React.ReactNode;}>) {
  ...
  return (
    <div className="mx-10">
        <section className="mt-10 mb-5 text-center xl:text-left">
        ...
        <div className="flex flex-col gap-4 xl:gap-10 xl:flex-row">
            <section className="w-full grid gap-2 h-24 xl:w-64">
                <Link href={'/dashboard/'} 
                className={
                    `btn btn-primary hover:bg-[#e7e7e9] rounded-md p-2 transition duration-200 text-center xl:text-left ${pathName === '/dashboard' ? 'bg-[#e7e7e9]' : ''}`
                }>Профиль</Link>
                <Link href={'/dashboard/account'} 
                className={
                    `btn btn-primary hover:bg-[#e7e7e9] rounded-md p-2 transition duration-200 text-center xl:text-left ${pathName === '/dashboard/account' ? 'bg-[#e7e7e9]' : ''}`
                    }>Аккаунт</Link>
            </section>
            <section className="">
                {children}
